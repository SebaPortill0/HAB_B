...
def set_chat_finished(state: State) -> bool:
    ...
    if len(parts) > 1:
        ...
        print(
            colored("Error: Could not find '>> FINAL ANSWER:' in the response", "red")
        )
...
class Jar3d(BaseAgent[State]):
    ...
    def get_prompt(self, state: State = None) -> str:
        system_prompt_md = read_markdown_file(
    ...
    def process_response(
        self, response: Any, user_input: str, state: State = None
...
class MetaExpert(BaseAgent[State]):
    ...
    def run(self, state: State) -> State:
        ...
        if (
            recursions >= lower_limit_recursions
            and recursions <= upper_limit_recursions
        ):
            final_answer = (
                "**You are being explicitly told to produce your [Type 2] work now!**"
            )
        ...
        formatted_requirements = "\n\n".join(
            re.findall(r"```python\s*([\s\S]*?)\s*```", requirements, re.MULTILINE)
        )
        ...
        if state.get("meta_prompt"):
            ...
            cor_match = "\n\n".join(
                re.findall(r"```python\s*([\s\S]*?)\s*```", meta_prompt, re.MULTILINE)
            )
        ...
        state = self.invoke(
            state=state, user_input=user_input, final_answer=final_answer
        )
...
class ToolExpert(BaseAgent[State]):
    ...
    def __init__(
        ...
        super().__init__(
            model, server, temperature, model_endpoint, stop, location, hybrid
        )
    ...
    def generate_search_queries(
        self, meta_prompt: str, num_queries: int = 5
    ) -> List[Dict[str, str]]:
        ...
        refine_prompt = refine_query_template.format(
            manager_instructions=meta_prompt, num_queries=num_queries
        )
        input_data = [
    ...
    def process_serper_result(
        self, query: Dict[str, str], serper_response: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        best_url_prompt = best_url_template.format(
            search_query=query["query"], serper_results=serper_response
        )
        input_data = [
    ...
    def run(self, state: State) -> State:
        ...
        max_iterations = (
            5  # Define a maximum number of iterations to prevent infinite loops
        )
        iteration = 0
        ...
        while iteration < max_iterations:
                ...
                with concurrent.futures.ThreadPoolExecutor(
                    ...
                    for future in concurrent.futures.as_completed(future_to_query):
                        ...
                        try:
                            ...
                            print(
                                colored(f"Error processing query {query}: {exc}", "red")
                            )
                            serper_results.append(None)
            except Exception as e:
                print(
                    colored(
                        f"Error in threading: {str(e)}. Falling back to non-parallel processing.",
                        "red",
                    )
                )
                serper_results = [
                    self.use_tool("serper", query["engine"], query["query"], None)
                    for query in search_queries
                ]
            ...
            try:
                with concurrent.futures.ThreadPoolExecutor(
                    max_workers=min(len(serper_results), 5)
                ) as executor:
                    future_to_query = {
                        executor.submit(
                    ...
                    for future in concurrent.futures.as_completed(future_to_query):
                        ...
                        try:
                            ...
                            print(
                                colored(
                                    f"Error processing result for query {query}: {exc}",
                                    "red",
                                )
                            )
                ...
                print(
                    colored(
                        f"Error in threading: {str(e)}. Falling back to non-parallel processing for best URLs.",
                        "red",
                    )
                )
                best_url_results = [
                    self.process_serper_result(
                        query, result["results"] if result else {}
                    )
                    for query, result in zip(search_queries, serper_results)
                ]
            ...
            print(
                colored(
                    "\n\n Sourced data from {} sources:".format(len(unique_urls)),
                    "yellow",
                )
            )
        ...
        try:
            ...
            scraper_response = {
                "results": f"Error {e}: Failed to scrape results",
                "is_shopping": False,
            }

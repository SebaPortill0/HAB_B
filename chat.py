...
from agents.jar3d_agent import (
    State,
    Jar3d,
    MetaExpert,
    Router,
...
def get_agent_kwargs(
    server: str = "claude", location: str = None, hybrid: bool = False
) -> Dict[str, Any]:
...
class Jar3dIntro(BaseAgent[State]):
    ...
    def run(self, state: State) -> State:
        ...
        jar3d_intro = re.sub(
            r"^```python[\s\S]*?```\s*", "", jar3d_intro, flags=re.MULTILINE
        )
...
async def update_settings(settings):
    ...
    agent_kwargs, agent_kwargs_tools, agent_kwargs_meta_expert = get_agent_kwargs(
        server, gl, hybrid
    )
    ...
    await cl.Message(
        content=f"Realizaré cualquier búsqueda en Internet desde {location}.",
        author="Jar3d👩‍💻",
    ).send()
...
async def start():
    ...
    await cl.Message(
        content=f"{jar3d_intro_hi}.\n\n Realizaré cualquier búsqueda en Internet desde los Estados Unidos.",
        author="Jar3d👩‍💻",
    ).send()
...
def build_workflow():
    ...
    graph.add_node(
        "no_tool_expert", lambda state: no_tool_expert_instance.run(state=state)
    )
...
async def main(message: cl.Message):
    ...
    loop = asyncio.get_running_loop()
    state, response = await loop.run_in_executor(
        None, agent.run_chainlit, state, message
    )
    ...
    if message.content == "/end":
        await cl.Message(content="Compilando Workflow...", author="System").send()
        final_answer = await cl.make_async(run_workflow)(workflow, state)
        if final_answer:
            await cl.Message(content=final_answer, author="Jar3d👩‍💻").send()
        else:
            await cl.Message(
                content="No se produjo una respuesta final.", author="Jar3d👩‍💻"
            ).send()
    else:

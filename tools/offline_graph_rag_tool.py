...
def index_and_rank(
    corpus: List[Document], query: str, top_percent: float = 20, batch_size: int = 25
) -> List[Dict[str, str]]:
    print(
        colored(
    ...
    # embeddings = FastEmbedEmbeddings(model_name='BAAI/bge-small-zh-v1.5', max_length=512, cache_dir=CACHE_DIR)
    # print(f"Model initialized: {embeddings._model is not None}")
            ...
            for chunk in doc.chunks():
                document = Document(
                    ...
                    metadata={
                        "source": url,
                        "query": query,
                    },  # Change: Added query to metadata
                )
...
def process_document(
    doc: Document, llm_transformer: LLMGraphTransformer, doc_num: int, total_docs: int
) -> List:
    print(
        colored(
...
def run_rag(
    ...
    retrieved_context = run_hybrid_graph_retrieval(
        graph=graph, corpus=corpus, query=query, hybrid=hybrid
    )
